...
impl<'a> Downloader<'a> {
    ...
    pub fn download(&self) -> Result<(), Error> {
        ...
        let jar_path = &self.server.location.join(format!("{}-{}.jar", &jar.name, &version));

        let jar_details = jars.jars.iter().find(|j| j.name == jar.name).ok_or(Error::JarNotFound)?;
        let download_url = &jar_details.download_url
